...
## 9: Why `virtual DOM` is faster to update than real DOM? ?
...
## 10: What would happen if we do `console.log(useState())`?
...
## 12: What are `refs` used for in React?
...
## 13: What is the difference between `Client Side Routing` and `Server Side Routing`?
...
## 14: When and why do we need `lazy()` in react application?
The `lazy()` function is used in React to dynamically load components only when they are needed, which can significantly improve the performance of your application. It's especially useful when dealing with larger applications with complex component hierarchies. Instead of loading all components upfront, you can use `lazy()` to load components asynchronously, thus reducing the initial bundle size and improving the initial load time of your application.

## 15: what is `Code Splitting` in react?
`Code splitting` is a technique used in React (and other modern web development frameworks) to improve the performance of web applications by splitting the application's codebase into smaller chunks that are loaded on-demand. Instead of loading the entire application's code upfront when a user visits a webpage, code splitting allows you to load only the necessary parts of the code when they are needed.
